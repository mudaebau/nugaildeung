-- ############################################################################
-- ##  🔴 폐기됨 — 블록 B를 다시 실행하지 마십시오. 파일럿 개설 이후 실행 금지.  ##
-- ############################################################################
--
--   이 파일의 블록 B는 "보존 5건이 아닌 대회를 전부 삭제"하는 화이트리스트
--   방식입니다. 화이트리스트가 데모 5건으로 고정돼 있어서,
--   **파일럿 실개설 대회(김진석 파크골프장 파일럿대 등)가 목록에 없습니다.**
--   지금 블록 B를 돌리면 파일럿 대회와 그 참가자·스코어가 전부 삭제됩니다.
--   안전장치(remain <> 5)도 못 막습니다 — 오히려 파일럿을 지워야 5가 되므로
--   "정상 통과"로 커밋됩니다. 안전장치가 거꾸로 작동하는 상태입니다.
--
--   이 파일은 1차 정리(2026-07-29, 실행 완료)의 기록으로만 남깁니다.
--   파일럿 이후 [TEST] 정리는 삭제할 id를 직접 지목하는 별도 SQL로 새로 만듭니다
--   (화이트리스트 방식 금지 — 실데이터가 목록에 없으면 지워지기 때문).
--
--   블록 A(읽기 전용 조회)와 블록 C(운영자, 미실행)는 그대로 참고 가능합니다.
-- ############################################################################

-- ============================================================================
--  파일럿 GO ① — 운영 DB 전면 정리  [1차 정리 완료분 · 재실행 금지]
...
--
-- ── (아래 2회 실행 계획은 파일럿 개설 전 기준입니다. 3단계까지 실행 완료했고,
--     4단계에서 파일럿이 개설된 순간부터 이 계획은 무효입니다 — 위 폐기 경고 참조) ──
