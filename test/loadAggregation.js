...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2564, 2565, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1682, 1689, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2341, 2389, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5235, 5253, 'function sums'),            // sums, standings
    extractBlock(lines, 5457, 5482, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
