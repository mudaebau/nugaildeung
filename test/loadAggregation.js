...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2534, 2535, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1669, 1676, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2311, 2359, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5116, 5134, 'function sums'),            // sums, standings
    extractBlock(lines, 5322, 5347, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
