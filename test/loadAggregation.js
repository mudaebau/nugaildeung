...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2768, 2769, 'const bestMap={}');
  ...
  const src = [
    ...
    extractBlock(lines, 2521, 2569, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5483, 5501, 'function sums'),            // sums, standings
    extractBlock(lines, 5705, 5730, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
