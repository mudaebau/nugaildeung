...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2771, 2772, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1752, 1759, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2524, 2572, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5486, 5504, 'function sums'),            // sums, standings
    extractBlock(lines, 5708, 5733, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
