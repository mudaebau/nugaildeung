...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2705, 2706, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1746, 1753, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2482, 2530, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5414, 5432, 'function sums'),            // sums, standings
    extractBlock(lines, 5636, 5661, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
