...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2774, 2775, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1755, 1762, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2527, 2575, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5489, 5507, 'function sums'),            // sums, standings
    extractBlock(lines, 5711, 5736, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
