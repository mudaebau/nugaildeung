...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2709, 2710, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1750, 1757, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2486, 2534, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5418, 5436, 'function sums'),            // sums, standings
    extractBlock(lines, 5640, 5665, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
