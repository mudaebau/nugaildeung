...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2780, 2781, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1760, 1767, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2533, 2581, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5504, 5522, 'function sums'),            // sums, standings
    extractBlock(lines, 5726, 5751, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
