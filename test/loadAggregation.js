...
function loadAggregation() {
  ...
  const bestMapBody = extractBlock(lines, 2552, 2553, 'const bestMap={}');
  const computeBestMapFn = `function computeBestMap(plRows){\n${bestMapBody}\nreturn bestMap}`;

  const src = [
    extractBlock(lines, 1676, 1683, 'const PRESET66'),           // PRESET66/54, coursePars, parAt, courseParTotal, isOut
    extractBlock(lines, 2329, 2377, 'function cutCompetitors'),  // cutCompetitors ~ resolveCut(+courseSeries/courseCompare/ageCompare/tieGroupAt)
    extractBlock(lines, 5183, 5201, 'function sums'),            // sums, standings
    extractBlock(lines, 5405, 5430, 'function periodStandings'), // periodStandings
    computeBestMapFn,
  ].join('\n\n');
